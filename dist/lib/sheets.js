...
export async function appendPaymentRow(session) {
    // Временно отключаем Google Sheets функциональность
    // TODO: Настроить Google Sheets API
    console.log('Google Sheets integration disabled - payment data:', {
        session_id: session.id,
        amount: (session.amount_total ?? 0) / 100,
        currency: session.currency,
        email: session.customer_details?.email || session.customer_email
    });
    // Раскомментируйте после настройки Google Sheets API:
    /*
