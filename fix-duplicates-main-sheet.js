// Скрипт для поиска и исправления дубликатов в основном листе payments
...
async function findAndFixDuplicates() {
  console.log('🚀 Запускаем поиск и исправление дубликатов в основном листе...\n');
  ...
  try {
    ...
    console.log(`📋 Найдено ${allRows.length} записей в основном листе\n`);
    ...
    for (const [customerId, rows] of customerGroups.entries()) {
      if (rows.length > 1) {
        duplicates.push({ customerId, rows, count: rows.length });
      }
    }

    console.log(`🔍 Найдено ${duplicates.length} клиентов с дубликатами\n`);
    ...
    for (const { customerId, rows } of duplicates) {
      try {
        ...
        const updatedRowData = formatPaymentForSheets(latestPayment, customer);
        await keepRow.save({
