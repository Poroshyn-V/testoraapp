...
async function fixRemainingData() {
  try {
    console.log('🔧 ИСПРАВЛЕНИЕ ОСТАВШИХСЯ ДАННЫХ');
    console.log('==================================');
    ...
    // Исправляем каждую запись с задержкой для избежания лимитов
    let fixedCount = 0;
    ...
    for (let i = 0; i < recordsToFix.length; i++) {
      ...
      try {
        // Добавляем задержку каждые 10 записей
        if (i > 0 && i % 10 === 0) {
          console.log(`⏳ Пауза 2 секунды для избежания лимитов API...`);
        ...
        if (error.message.includes('Quota exceeded')) {
          console.log(`⏳ Лимит API превышен, ждем 5 секунд...`);
...
console.log('🚀 Запуск исправления оставшихся данных...');
fixRemainingData();
