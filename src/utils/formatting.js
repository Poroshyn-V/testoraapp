...
// Format notification message for Telegram (OLD COMPACT FORMAT)
export function formatTelegramNotification(payment, customer, metadata = {}) {
  ...
  // Create OLD COMPACT notification message
...
// Format notification message for Slack (SAME AS TELEGRAM - OLD FORMAT)
export function formatSlackNotification(payment, customer, metadata = {}) {
  ...
  // Create OLD COMPACT notification message (SAME AS TELEGRAM)
  const message = `🟢 Order ${payment.id.substring(0, 7)} was processed!
---------------------------
💳 card
💰 ${amount} ${currency}
