...
app.listen(ENV.PORT, () => {
  ...
  // Then every 2 minutes
        ...
        // ИСПРАВЛЕННАЯ АВТОМАТИЗАЦИЯ - РАБОТАЕТ НА VERCEL
        console.log('🔧 ИСПРАВЛЯЮ ПРОБЛЕМУ: Настраиваю надежную автоСинхронизацию...');
        ...
        setInterval(async () => {
          try {
            ...
            await runSync();
            console.log('✅ АвтоПроверка завершена успешно');
          } catch (error) {
        ...
        setInterval(async () => {
          try {
            ...
            await runSync();
            console.log('✅ Дополнительная проверка завершена');
          } catch (error) {
            console.error('❌ Ошибка дополнительной проверки:', error.message);
          }
        }, 2 * 60 * 1000);
        
        // ПРИНУДИТЕЛЬНАЯ АКТИВНОСТЬ каждые 30 секунд чтобы Vercel не засыпал
        setInterval(() => {
          console.log('💓 ПОДДЕРЖИВАЮ АКТИВНОСТЬ: Vercel не заснет...');
        }, 30 * 1000);
});
